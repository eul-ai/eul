...
The OpenAI adapter generically parses incomplete JSON argument prefixes for display only. `write` uses the resulting partial argument object to preview at most ten lines and 4 KiB while the model generates content, reports omitted and total line counts when line-bounded, but executes only once with the complete, strictly decoded raw arguments. Subagent execution similarly replaces a compact child-status list, updates running elapsed times once per second, and shows each child's cumulative reported token usage while it runs without streaming full child transcripts. One-shot output ignores intermediate replacement snapshots and remains append-only.

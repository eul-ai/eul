...
func (c *tuiController) transition(ctx context.Context, event tuiEvent) (bool, error) {
	...
	case tuiEventClipboardImage:
		return c.handleClipboardImage(event.image, event.err)
...
func (c *tuiController) handleClipboardImage(image agent.Image, err error) (bool, error) {
	...
	if err := c.model.attachImage(image); err != nil {
		setInputError(c.model, err)
	}
	c.dirty = true
	return false, nil
...
func (c *tuiController) applyAction(ctx context.Context, action tuiAction) (bool, error) {
	...
	case tuiActionSubmit:
		return false, c.startTurn(ctx, action.prompt, action.images)
	case tuiActionSteer:
	...
	case tuiActionSetGoal:
		...
		return false, c.startTurn(ctx, action.prompt, nil)
...
func loadClipboardImage(ctx context.Context, read func(context.Context) (agent.Image, error), events chan<- tuiEvent) {
	image, err := read(ctx)
	events <- tuiEvent{kind: tuiEventClipboardImage, image: image, err: err}
}

func (c *tuiController) startTurn(ctx context.Context, prompt string, images []agent.Image) error {
	c.model.beginTurnWithImages(prompt, len(images))
...
func (c *tuiController) startDeferredTurn(ctx context.Context) error {
	...
	return c.startTurn(ctx, prompt, nil)
}

...
func TestTUIControllerSubmitsClipboardImageAfterCheckpoint(t *testing.T) {
	...
	engine := &fakeEngine{runWithImagesFunction: func(_ context.Context, prompt string, images []agent.Image, _ agent.EventSink) (agent.RunResult, error) {
		if prompt != "describe" || len(images) != 1 || images[0].MediaType != "image/png" || string(images[0].Data) != "png" {
	...
	controller := tuiController{
		...
		saveCheckpoint: func(_ agent.Checkpoint, checkpoint Checkpoint, active bool) error {
			if !active || len(checkpoint.data.Blocks) != 1 || checkpoint.data.Blocks[0].ImageCount != 1 {
	...
	if _, err := controller.applyAction(context.Background(), tuiAction{
		kind: tuiActionSubmit, prompt: "describe",
		images: []agent.Image{{MediaType: "image/png", Data: []byte("png")}},
	}); err != nil {
		t.Fatal(err)
	}

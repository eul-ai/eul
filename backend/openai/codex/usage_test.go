...
func TestClientUsageGetsSubscriptionWindows(t *testing.T) {
	...
	want := agent.ProviderUsage{Windows: []agent.UsageWindow{
...
func TestClientUsageHandlesOptionalAndInvalidWindows(t *testing.T) {
	tests := []struct {
		name    string
		body    string
		want    agent.ProviderUsage
		wantErr string
	}{
		{name: "no rate limit", body: `{"plan_type":"plus","rate_limit":null}`},
		{name: "one window", body: `{"rate_limit":{"primary_window":{"used_percent":0,"limit_window_seconds":3600}}}`, want: agent.ProviderUsage{Windows: []agent.UsageWindow{{Duration: time.Hour}}}},
